...
# Ensure folders exist when the server starts
KB_DIR.mkdir(parents=True, exist_ok=True)
TICKETS_FILE.parent.mkdir(parents=True, exist_ok=True)

# Initialize with empty JSON object (dict) matching your backend tickets setup
if not TICKETS_FILE.exists():
    TICKETS_FILE.write_text("{}") 

mcp = FastMCP("IT Service Desk Tools")
...
def search_knowledge_base(query: str) -> str:
    """
    Search the local data/knowledge_base folder for IT articles.
    IMPORTANT: Pass short, specific keywords (e.g., 'vpn', 'slow laptop', 'password') instead of full sentences.
    """
    results = []
...
@mcp.tool()
def get_all_open_tickets() -> str:
    """
    Reads the data/tickets/tickets.json file and returns all currently open IT tickets.
    """
    try:
...
def create_it_ticket(
    ...
    """
    Creates a new IT ticket and saves it directly to the data/tickets/tickets.json dictionary.
    """
...
def get_ticket_status(ticket_id: str) -> str:
    """
    Fetch the details and current status of a specific IT ticket by its ID.
    """
    try:
        tickets = json.loads(TICKETS_FILE.read_text(encoding="utf-8"))
...
def escalate_ticket(ticket_id: str, reason: str) -> str:
    """
    Escalate a ticket to high priority when a user is blocked or reporting an urgent issue.
    """
    try:
        tickets = json.loads(TICKETS_FILE.read_text(encoding="utf-8"))
        tid = ticket_id.upper()
